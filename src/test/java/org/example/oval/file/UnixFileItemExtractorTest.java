...
public class UnixFileItemExtractorTest {


    private static final String basePath = "target/temp/fileExtractorTest";
    private static final String basePath2 = "target/temp/fileExtractorTest2";
    ...
    public void testPathCaseInsensitiveEquals() throws Exception {
        for (int i = 0; i< testItemCount; i++) {
            ...
            assert filepath.equalsIgnoreCase(basePathFile.getAbsolutePath() + "/" + filename);
        }
    }
    @Test
    public void testPathPatternMatch() throws Exception {
        for (int i = 0; i< testItemCount; i++) {
            ...
            for (FileItem fileItem : fileItems) {
                ...
                result.put(filepath.replace("/./", "/"), fileItem);
            }

            File basePathFile = new File(basePath);
            File basePathFile2 = new File(basePath2);
            assert result.containsKey(basePathFile.getAbsolutePath() + "/" + filename);
            assert result.containsKey(basePathFile2.getAbsolutePath() + "/" + filename);
        }
    }
    @Test
